...
from blingalytics.sources import sqlalchemy, derived, key_range, merge, static
...
class BasicDatabaseReport(base.Report):
    ...
    filters = [
        ('user_is_active', sqlalchemy.QueryFilter(lambda entity, user_input: entity.user_is_active == user_input if user_input is not None else None,
            widget=widgets.Select(label='User Is Active', choices=ACTIVE_USER_CHOICES))),
    ]
    keys = ('user_id', key_range.SourceKeyRange)
    columns = [
        ('user_id', sqlalchemy.GroupBy('user_id', format=formats.Integer(label='User ID', grouping=False))),
        ('user_is_active', sqlalchemy.First('user_is_active', format=formats.Boolean(label='Active?'))),
        ('num_widgets', sqlalchemy.Count('widget_id', distinct=True, format=formats.Integer(label='Widgets'))),
        ('_sum_widget_price', sqlalchemy.Sum('widget_price')),
...
class BasicMergeReport(base.Report):
    ...
    filters = [
        ...
        ('user_is_active', merge.DelegatedFilter(
            sqlalchemy.QueryFilter(lambda entity, user_input: entity.user_is_active == user_input if user_input is not None else None),
            widget=widgets.Select(label='User Is Active', choices=ACTIVE_USER_CHOICES))),
