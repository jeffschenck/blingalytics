...
from blingalytics.sources import sqlalchemy
...
class TestSQLAlchemySource(unittest.TestCase):
    ...
    def test_sqlalchemy_source(self):
        source = sqlalchemy.SQLAlchemySource(self.report)
    ...
    def test_sqlalchemy_key_ranges(self):
        # Straight up
        key_range = sqlalchemy.TableKeyRange('test.entities.AllTheData', pk_column='widget_id')
        self.assertEqual(set(key_range.get_row_keys([])), set([1, 2, 3, 4]))
    
        # Now with filtering
        key_range = sqlalchemy.TableKeyRange('test.entities.AllTheData', pk_column='widget_id',
            filters=sqlalchemy.QueryFilter(lambda entity: entity.id > 2))
        self.assertEqual(set(key_range.get_row_keys({})), set([3, 4]))
    
    def test_sqlalchemy_filters(self):
        # ColumnTransform functionality
        self.assertRaises(ValueError, sqlalchemy.ColumnTransform, lambda column: column.op('+')(1))
        fil = sqlalchemy.ColumnTransform(lambda column: column.op('+')(1), columns=['plussed'])
        ...
        widget._name = 'widget'
        fil = sqlalchemy.ColumnTransform(lambda column, user_input: column.op('+')(user_input), columns=['plussed'], widget=widget)
        ...
        fil = sqlalchemy.QueryFilter(lambda entity: entity.id < 10)
        ...
        widget._name = 'widget'
        fil = sqlalchemy.QueryFilter(lambda entity, user_input: entity.id.in_(user_input) if user_input else None, widget=widget)
    ...
    def test_sqlalchemy_columns(self):
        ...
        col = sqlalchemy.Lookup('test.entities.AllTheData', 'user_id', 'widget_id')
        ...
        col = sqlalchemy.Lookup('test.entities.AllTheData', 'user_id', 'widget_id', 'widget_id')
        self.assert_(col.pk_attr is entities.AllTheData.widget_id)

        # GroupBy
        col = sqlalchemy.GroupBy('user_id')
